...
- **`LevelMomentRewardedAd.load(...)`** — static factory mirrors `RewardedAd.load(adUnitId, request, callback)`; synchronous mark-ready (no native preload), supports `format` (`'flashcard'`, `'quiz'`, `'deep_dive'`)
...
- **postMessage bridge** — terminal-once dismiss semantics (rewards may repeat; `dismissed`/`error` fire dismissal exactly once)
