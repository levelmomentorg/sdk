...
- **`subscription_required` error code** — a load refused because the parent's subscription lapsed surfaces as `code: "subscription_required"` (parsed from the API's 403 body, including when the silent device→session re-exchange is what got refused) instead of collapsing to `invalid_token`/`unknown`. The hosted `/break` page uses it to show an ask-a-parent recovery screen; treat it like `no_fill` in game code.
...
- [ ] `_submitAnswer` has no retry queue — failures are classified (transport/429/5xx → `retriable`, other 4xx → terminal `unrecorded` skip via `skipCurrentQuestion()`) but a dropped answer is never re-submitted later
...
- **`requestTimeoutMs`** is applied via `AbortSignal.timeout` to every outbound fetch — a stalled connection no longer hangs the SDK when an ad slot opens. The default (10 000 ms) is intentionally conservative; games that use a fast CDN may want to lower it.
