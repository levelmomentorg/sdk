...
export interface AdvanceResult {
  ...
  retriable?: boolean;
  /**
   * True when the answer was never graded server-side and retrying would fail
   * identically (a terminal 4xx — e.g. a rejected impression id), so the flow
   * moved past the question instead. `outcome` is "skipped"; award no reward
   * and show no wrong-answer feedback for it.
   */
  unrecorded?: boolean;
}
...
export type LevelMomentAdErrorCode =
  ...
  | "no_fill"
  | "invalid_token"
  | "subscription_required" // credential is fine; the parent's subscription lapsed
  | "not_loaded"
  | "unknown";
...
export interface LevelMomentAdHandle {
   ...
   * hatch from a persistent submit failure. Returns an unrecorded "skipped"
   * result (no reward, no score change) with the next question, or null when
   * the session is complete / the format is flashcard.
   */
