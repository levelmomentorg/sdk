...
describe("LevelMomentAd — silent device→session exchange (POST /sessions)", () => {
  ...
  it("surfaces subscription_required on a direct 403 from /questions", async () => {
    const fetchMock = makeFetch([
      {
        match: "/questions",
        respond: () =>
          jsonResponse(403, {
            error: {
              ...
              message: "An active subscription is required to play",
            },
          }),
      },
    ]);
  ...
  it("surfaces subscription_required on a direct 403 from /break-sessions", async () => {
    const fetchMock = makeFetch([
      {
        match: "/break-sessions",
        respond: () =>
          jsonResponse(403, {
            error: { code: "subscription_required", message: "…" },
          }),
      },
    ]);
  ...
  it("surfaces subscription_required when the device exchange is refused for a lapsed subscription", async () => {
    // A paired device whose session rows were deleted by the gate: /questions
    // 401s (session gone), the device re-exchange then hits the gate's 403.
    // Without the distinct code this surfaced as invalid_token and hosts
    // rendered a dead end instead of an ask-a-parent screen.
    const fetchMock = makeFetch([
      {
        match: "/questions",
        respond: () => jsonResponse(401, { error: { code: "invalid_token" } }),
      ...
      LevelMomentAd.load(
        {
          apiUrl: API_URL,
          placementId: PLACEMENT_ID,
          studentToken: "stored-device-token",
        },
        makeQueue(),
        {
          onAdLoaded: () => resolve({ code: "unexpected_load" }),
...
// ---------------------------------------------------------------------------
// Answer-submission failure classes: a terminal 4xx must NOT trap the student
// on the retry card (the old bug: every non-2xx surfaced as retriable, so a
// 403 for a rejected impression id looped "try again" forever with a false
// "check your connection" diagnosis).
// ---------------------------------------------------------------------------

const QUIZ_SESSION_BODY = {
  sessionId: "sess-fail",
  format: "quiz",
  questions: [
    {
...
async function loadQuizAd(fetchMock: FetchMock): Promise<LevelMomentAd> {
  return new Promise<LevelMomentAd>((resolve, reject) => {
    LevelMomentAd.load(
      {
        apiUrl: API_URL,
...
describe("LevelMomentAd — submit failure classes (retriable vs terminal)", () => {
  it("advanceSession treats a 403 from /answers as a terminal unrecorded skip and advances", async () => {
    const fetchMock = makeFetch([
      {
        match: "/break-sessions",
        respond: () => jsonResponse(200, QUIZ_SESSION_BODY),
      },
      {
        match: "/answers",
    ...
    const ad = await loadQuizAd(fetchMock);
    ...
    expect(result.outcome).toBe("skipped");
    // The flow moved PAST q-1 — the same question is not re-served.
    expect(result.next?.id).toBe("q-2");
  });

  it("advanceSession keeps a 500 from /answers retriable with the same question", async () => {
    const fetchMock = makeFetch([
      {
        match: "/break-sessions",
        respond: () => jsonResponse(200, QUIZ_SESSION_BODY),
      ...
      {
        match: "/answers",
        respond: () => jsonResponse(500, { error: { code: "internal_error" } }),
      },
    ]);
    const ad = await loadQuizAd(fetchMock);
  ...
  it("submitFlashcardAnswer treats a 403 as terminal (unrecorded, not retriable)", async () => {
    const fetchMock = makeFetch([
      {
        match: "/questions",
        respond: () =>
          jsonResponse(200, {
            question: { id: "q-f", meta: { type: "multiple_choice" } },
          }),
      },
      {
        match: "/answers",
    ...
    const result = await ad.submitFlashcardAnswer({
      answeredAt: new Date().toISOString(),
      selectedIndex: 0,
    });
  ...
  it("skipCurrentQuestion advances the session and completes it at the end", async () => {
    const fetchMock = makeFetch([
      {
        match: "/break-sessions",
        respond: () => jsonResponse(200, QUIZ_SESSION_BODY),
    ...
    const first = ad.skipCurrentQuestion();
    expect(first.outcome).toBe("skipped");
    expect(first.unrecorded).toBe(true);
    expect(first.next?.id).toBe("q-2");

    const second = ad.skipCurrentQuestion();
    expect(second.next).toBeNull();
    // Session completion PATCH fired (fire-and-forget) — allow the microtask.
    await new Promise((r) => setTimeout(r, 0));
    const patches = fetchMock.calls.filter(
      (c) =>
        c.url.includes("/break-sessions/sess-fail") &&
        c.init?.method === "PATCH",
    );
