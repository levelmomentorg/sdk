...
/**
 * Best-effort read of the API's structured error code from a non-ok response
 * body (`{ error: { code, message } }`). Returns null for non-JSON bodies or
 * unexpected shapes. Consumes the body — call only on responses whose body
 * is not otherwise read.
 */
async function errorCodeFromBody(res: Response): Promise<string | null> {
  try {
...
export class LevelMomentAd implements LevelMomentAdHandle {
  ...
  /** Return a copy of `init` with an `AbortSignal.timeout` applied. */
  private _withTimeout(init?: RequestInit): RequestInit {
    return {
      ...init,
  ...
  noteQuestionShown(): void {
    ...
    const event: ImpressionEvent = {
      ...
      durationMs: 0,
      idempotencyKey,
      // SSV-parity: carry the ad's customData onto every impression it records
      // so the server can echo it on reward.earned (mirrors AdMob SSV). Only
      // set when configured — the server caps it at 1024 chars.
      ...(this.config.customData ? { customData: this.config.customData } : {}),
    };
  ...
  async advanceSession(answer: SessionAnswer): Promise<AdvanceResult> {
    ...
    // Terminal failure — a retry would replay the identical error, so move
    // past the question as an unrecorded skip instead of trapping the student
    // on a retry card.
    if (graded === "terminal") {
      return this.skipCurrentQuestion();
    }
  ...
  async submitFlashcardAnswer(answer: SessionAnswer): Promise<AdvanceResult> {
    ...
    // Terminal failure → end the flashcard as an unrecorded skip (no reward).
    if (graded === "terminal") {
      return { outcome: "skipped", next: null, unrecorded: true };
    }
    return toAdvanceResult(graded);
  }

  /**
   * Move past the current question without submitting an answer — the escape
   * hatch from the retry card (persistent network failure) and the fallback
   * for terminal submit errors. The server never grades the question, so the
   * result is an unrecorded "skipped": no reward, no score change. For
   * sessions it advances to the next question (completing the session at the
   * end); for flashcards it just ends the break.
   */
  skipCurrentQuestion(): AdvanceResult {
    if (!this._session) {
      return { outcome: "skipped", next: null, unrecorded: true };
    }
    ...
    if (this._currentIndex >= this._session.questions.length) {
      void this._completeSession(this.getSessionSummary());
      return { outcome: "skipped", next: null, unrecorded: true };
    }
  ...
  // The parent portal's Approve-device flow hands players a long-lived
  // DEVICE token, and POST /sessions documents the session mint as a silent
  // SDK responsibility. When the API rejects the configured token (401), try
  // it as a device token exactly once: exchange it for a game-scoped session
  // token and retry the original request. Session tokens never pay this cost
  // (no 401, no exchange), and a genuinely invalid token still surfaces as
  // invalid_token after the failed exchange. On success the shared config's
  // studentToken is replaced, so answer submission and the impression-queue
  // flush (which read the same config object) use the minted session token.
  private _deviceExchangeTried = false;

  // Set when the device-token exchange is refused because the parent's
  // subscription lapsed (403 subscription_required from POST /sessions).
  // Without this, a paired device with a dead session token surfaces the
  // lapse as a generic invalid_token and hosts show a dead end instead of
  // an "ask a parent" screen.
  private _exchangeRefusedSubscription = false;

  private async _tryDeviceExchange(): Promise<boolean> {
    if (this._deviceExchangeTried) return false;
    this._deviceExchangeTried = true;
    try {
      const res = await this.fetchFn(
        ...
        this._withTimeout({
          ...
          body: JSON.stringify({
            placementId: this.config.placementId,
            deviceToken: this.config.studentToken,
          }),
        }),
      );
      if (!res.ok) {
        if ((await errorCodeFromBody(res)) === "subscription_required") {
          this._exchangeRefusedSubscription = true;
        }
        return false;
      }
      ...
      this.config.studentToken = data.sessionToken;
      return true;
    } catch {
      return false;
    }
  ...
  /**
   * Map a failed /questions or /break-sessions response to an ad error.
   * `subscription_required` (403, or a 401 whose device-token re-exchange was
   * itself refused for a lapsed subscription) is surfaced distinctly so hosts
   * can show an "ask a parent" recovery screen; a plain 401 after a failed
   * exchange remains `invalid_token`.
   */
  private async _loadFailureError(res: Response): Promise<LevelMomentAdError> {
    let code: LevelMomentAdError["code"];
    if (this._exchangeRefusedSubscription) {
      code = "subscription_required";
    } else if (res.status === 401) {
      code = "invalid_token";
    } else if ((await errorCodeFromBody(res)) === "subscription_required") {
      code = "subscription_required";
    } else {
      code = "unknown";
    }
  ...
  private async _fetchFlashcard(callbacks: AdLoadCallback): Promise<void> {
    ...
    const res = await this.fetchFn(
      ...
      this._withTimeout({ headers: buildAuthHeader(this.config.studentToken) }),
    );

    if (!res.ok) {
      if (res.status === 401 && (await this._tryDeviceExchange())) {
        return this._fetchFlashcard(callbacks);
      }
      callbacks.onAdFailedToLoad(await this._loadFailureError(res));
      return;
    }
  ...
  private async _fetchSession(
    ...
    const res = await this.fetchFn(
      `${this.config.apiUrl}/break-sessions`,
      this._withTimeout({
        method: "POST",
        headers: {
    ...
    if (!res.ok) {
      if (res.status === 401 && (await this._tryDeviceExchange())) {
        return this._fetchSession(callbacks, format);
      }
      callbacks.onAdFailedToLoad(await this._loadFailureError(res));
      return;
    }
  ...
  private async _postImpressionForId(event: ImpressionEvent): Promise<string> {
    try {
      const res = await this.fetchFn(
        `${this.config.apiUrl}/impressions`,
        this._withTimeout({
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  /**
   * POST the answer and grade it. Distinguishes the two failure classes so
   * callers don't loop a student forever on an error a retry can never fix:
   *   - "retriable": transport failure, timeout, 429, or 5xx — the same
   *     request may succeed if repeated.
   *   - "terminal": any other non-2xx (or a 2xx with no outcome) — e.g. a 403
   *     for a rejected impression id; retrying replays the identical failure.
   */
  private async _submitAnswer(
    answer: Partial<SessionAnswer> & { impressionId?: string },
  ): Promise<GradedAnswer | "retriable" | "terminal"> {
    try {
      const res = await this.fetchFn(
        `${this.config.apiUrl}/answers`,
        this._withTimeout({
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      return data.outcomes?.[0] ?? "terminal";
    } catch {
      return "retriable";
    }
  }
