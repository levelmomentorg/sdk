...
**Status: ✅ Complete — thin loader over the hosted `/break` page; ships no DOM renderer**
...
- **`WebClientConfig`** — `LevelMomentConfig & { breakUrl?: string; mock?: boolean; breakLoadTimeoutMs?: number }`
...
- **Unit tests** — URL building (mock/live, breakUrl default/override, `?`/`&`), the postMessage bridge (reward mapping, dismiss/error teardown, origin mismatch ignored), `start()/stop()` no-ops, iframe mounting/teardown, the load-timeout watchdog (fires on host silence, cleared by `ready`/`dispose()`/`dismissed`, custom + `0`-disabled timeout)
...
pendingAd?.show({
  onUserEarnedReward: (reward) => reward.amount === 1 && grantBonus(),
