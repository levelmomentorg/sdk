...
The skill runs locally. It only contacts LevelMoment's API when:

- You run `/levelmoment register` (creates a game on your developer account)
- You opt-in to auto-fetch a placement ID during `/levelmoment port`

Auth tokens are stored at `~/.levelmoment/auth.json` with `chmod 600`. Revoke
anytime at `https://app.levelmoment.com/settings/cli-tokens`.
