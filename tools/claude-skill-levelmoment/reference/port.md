...
4. Inspect the specified slot and replace only its rewarded lifecycle. Keep
   ...
   ownership.
5. Use a new ad handle for every show. Grant the supplied game-owned reward
   action once for a server-confirmed terminal `rewardId`, then resume once on
   dismissal, show failure, or load failure.
